...
function getForgotPasswordEmailTemplate(userName, resetToken) {
    ...
    // Kullanıcı bu linke tıklayınca /reset-password#token=... sayfasına gider.
    ...
    // Frontend bu sayfayı render ettikten sonra token hidden input'a taşınmalı
    const resetLink  = `${DOMAIN}/reset-password#token=${encodeURIComponent(resetToken)}`;
    return `<!DOCTYPE html>
<html lang="tr">
<head>
<meta charset="UTF-8">
...
function getLoginNotificationTemplate(userName, loginDetails, resetToken = null) {
    ...
    <div style="background:rgba(255,152,0,0.08);border:1px solid rgba(255,152,0,0.2);border-radius:14px;padding:16px 20px;margin-top:16px;font-size:13px;color:rgba(255,255,255,0.6);line-height:1.7">
      ...
      Bu girişi siz yapmadıysanız <a href="${DOMAIN}/reset-password#token=${encodeURIComponent(resetToken)}" style="color:#00e676;font-weight:700">buraya tıklayarak</a> şifrenizi hemen sıfırlayın.
    </div>` : '';
    return `<!DOCTYPE html>
<html lang="tr">
<head>
    ...
    try {
        ...
        sendEmail(user.email, '⚠️ Agrolink — Şüpheli Giriş Bildirimi',
            `<p>Hesabınıza şüpheli bir giriş yapıldı ve siz bunu bildirdiniz.</p>
             ...
             <p>Şifre sıfırlama bağlantısı: <a href="${process.env.APP_URL || "https://sehitumitkestitarimmtal.com"}/reset-password#token=${encodeURIComponent(resetToken)}">Buraya tıklayın</a></p>`
        ).catch(() => {});
